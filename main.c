...
void Aide()
{
    ...
    printf("9. Les navires peuvent se deplacer a condition qu il ne soit pas couler et qu il ne se deplace que dans une direction verticale et horizontale d une seule case\n");
...
short checkShot (Tile gameBoard[][COLS], Coordinate target)
{
    ...
    if (gameBoard[target.row][target.column].symbol == 'C' || gameBoard[target.row][target.column].symbol == 'R' || gameBoard[target.row][target.column].symbol == 'S' ||
...
int main(int argc, char const *argv[])
{
    ...
    do
    {
        ...
        if (option == 1)
        {
            ...
            short sunkShip[2][NUM_OF_SHIPS] = {{1, 2, 4, 3},
                                               {1, 2, 4, 3}};  /* tracks parts of the ship destroyed */
